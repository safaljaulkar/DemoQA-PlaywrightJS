...
test('Verifying Form Functionality', async ({ page }) => {
  ...
  await test.step('Verify Form Fuctionality', async () => {
    await formAction.verifyFormFunctionality(commonData.firstname, commonData.lastname, commonData.email, commonData.mobileNumber, formData.subject1, formData.subject2, formData.subject3, formData.selectedFile, commonData.currentAddress);
